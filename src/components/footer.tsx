...
const Footer = () => {
  ...
  return (
    <footer className="w-full mt-20 rounded-t-3xl" style={{ background: 'linear-gradient(#faf7f5, #fff)' }}>
              ...
              <nav className="flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-2 ">
                {productLinks.map((link) => (
                  <Link
                    key={link.name}
                    href={link.href}
                    className="text-sm text-black hover:text-blue-600 transition-colors duration-300"
                  >
                    {link.name}
                  </Link>
                ))}
        ...
        <div className="border-t border-gray-200">
          <div className="flex flex-col sm:flex-row justify-between items-center py-4 space-y-4 sm:space-y-0">
            <p className="text-sm text-gray-600 text-center sm:text-left">
              &copy; {new Date().getFullYear()} PDF Gadgets. All rights reserved.
            </p>
            <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
              <Link href="/terms" className="text-sm text-gray-600 hover:text-[#5cacb3] transition-colors duration-300">
                Terms & Conditions
              </Link>
              <Link href="/privacy" className="text-sm text-gray-600 hover:text-[#5cacb3] transition-colors duration-300">
                Privacy Policy
              </Link>
              <Link href="/cookies" className="text-sm text-gray-600 hover:text-[#5cacb3] transition-colors duration-300">
                Cookies
              </Link>
            </div>
          </div>
